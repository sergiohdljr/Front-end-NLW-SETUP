...
export function NewHabitForm() {
 ...
 function handleToggleWeekDay (weekDay:number) {
   if(weekDays.includes(weekDay)){ 
    const weekDaysWithRemovedOnd = weekDays.filter(day => day !== weekDay)
    setWeekDays(weekDaysWithRemovedOnd)
   }else{
    const weekDaysWithEditOne = [...weekDays,weekDay]
    setWeekDays(weekDaysWithEditOne)
   }
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <div className="flex flex-col gap-2 mt-3">
        {availableWeekDays.map((DayAvalible, index) => {
          return (
            <Checkbox.Root
              key={DayAvalible}
            ...
            >
              ...
              <span className="text-white leading-tight">{DayAvalible}</span>
            </Checkbox.Root>
          );
        })}
      </div>
